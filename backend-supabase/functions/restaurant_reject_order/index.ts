...
serve(async (req) => {
  try {
    ...
    if (order.status !== "reserved") {
      return new Response(JSON.stringify({ error: `Order must be reserved to reject (current: ${order.status})` }), { status: 409 });
    }

    const { error: rpcErr } = await supabase.rpc("release_inventory_for_order", { p_order_id: orderId });
